import {
  ...
  export default function SearchGEBC({navigation, route}) {
    ...
    const fetchItemData = async search => {
      ...
      try {
        const response = await fetch(
          base_url + `gebc/searchGEBCs?name=${search}`,
          {
            method: 'GET',
            headers: {
        ...
        console.log('AllItems for ', result.GEBCs);
        setData(result.GEBCs); // Update the state with the fetched data
        setSearchTerm('');
        fetchAll();
      } catch (error) {
    ...
    useEffect(() => {
      const fetchData = async () => {
        ...
        try {
          const response = await fetch(
            base_url + `gebc/searchGEBCs?name=${selectedItemId}`,
            {
              method: 'GET',
              headers: {
                Authorization: `Bearer ${token}`,
          ...
          const result = await response.json();
          console.log('AllItems', result.GEBCs);
          setData(result.GEBCs); // Update the state with the fetched data
        } catch (error) {
          console.error('Error Trending:', error);
        }
    ...
    const loadSearchesFromStorage = async () => {
      try {
        const savedSearches = await AsyncStorage.getItem('gebcsearch');
        if (savedSearches) {
      ...
      return (
        <TouchableOpacity
          ...
          style={{
            ...
            <Text style={{fontSize: hp(10), // Make sure the image is square
      height: "100%", borderRadius:10, borderWidth:.5, borderColor:'gray'}}>{item?.image}</Text>
