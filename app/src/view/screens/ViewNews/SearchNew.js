  ...
  export default function SearchNews({navigation, route}) {
    ...
    const fetchItemData = async search => {
      ...
      try {
        const response = await fetch(
          base_url + `news/searchNews?name=${search}`,
          {
            method: 'GET',
            headers: {
        ...
        console.log('AllItems for ', result.News);
        setData(result.News); // Update the state with the fetched data
        setSearchTerm('');
        fetchAll();
      } catch (error) {
    ...
    useEffect(() => {
      const fetchData = async () => {
        ...
        try {
          const response = await fetch(
            base_url + `news/searchNews?name=${selectedItemId}`,
            {
              method: 'GET',
              headers: {
                Authorization: `Bearer ${token}`,
          ...
          const result = await response.json();
          console.log('AllItems', result.News);
          setData(result.News); // Update the state with the fetched data
        } catch (error) {
          console.error('Error Trending:', error);
        }
    ...
    const loadSearchesFromStorage = async () => {
      try {
        const savedSearches = await AsyncStorage.getItem('newssearch');
        if (savedSearches) {
